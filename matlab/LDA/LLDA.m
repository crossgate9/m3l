function [mappedX, mapping] = LLDA(X, labels, no_dims)
%LLDA Perform the local LDA algorithm
%
%   [mappedX, mapping] = LLDA(X, labels, no_dims)
%
% The function runs LLDA on a set of datapoints X. The variable
...
%
	...
	for i=1:nc
        ...
        % Get the means within class i
        mean_X(i,:) = mean(cur_X, 1);
        num_per_class(i) = size(cur_X,1);
    ...
    for i = 1:nc-1
        for j = i+1:nc
            wij = num_per_class(i)*num_per_class(j)/((mean_X(i,:)-mean_X(j,:))*(mean_X(i,:)-mean_X(j,:))');
            Sb = Sb + wij*(mean_X(i,:)-mean_X(j,:))'*(mean_X(i,:)-mean_X(j,:));
        end
    end
